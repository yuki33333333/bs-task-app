el-training
このカリキュラムについて
この文書は、Ruby on Railsとその周辺技術の基礎を習得するための新入社員教育用カリキュラムです。 新入社員の能力によらず、技術本部に属する社員は必ず一通りのステップを実施していただきます。 研修期間は特に定めておりません。 すべてのステップを完了した時点で研修修了となります。
...
その上で空のリポジトリを作成しましょう
環境構築用リポジトリを自分のアカウントにfolkしてきましょう
...
※ 現時点で正解のモデル図を作成する必要はまだありません。現時点での想定として作ってみましょう（今後のステップで間違いと思ったら改修していくイメージです） ※ RailsではActiveRecord（パターン）を使用するため、基本的にモデルとテーブルが１：１になりますが、そうではないケースもあります。気になる場合は参考としてコーチに質問してみましょう
...
GitHub上でPRを作成してレビューしてもらいましょう
PRの概要に叩いたrails generate コマンドを貼り付けて説明書きを入れましょう
...
GitHub上でPRを作成してレビューしてもらいましょう
PRの概要に叩いたrails generate コマンドを貼り付けて
...
１週間ほど
（番外編）オプション要件
必須要件とは別に、タスク管理システムに関するオプション要件を以下に示します。 コーチと相談しつつ必要に応じて実施してみてください。
